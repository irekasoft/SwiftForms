...
open class FormBaseCell: UITableViewCell {
  ...
  open func inputAccesoryView() -> UIToolbar {
    ...
    let clearButton = UIBarButtonItem(barButtonSystemItem: UIBarButtonItem.SystemItem.stop, target: self, action: #selector(FormBaseCell.handleClearAction(_:)))
    clearButton.tintColor = UIColor.red
    
    let doneButton = UIBarButtonItem(title: "Select", style: .done, target: self, action: #selector(FormBaseCell.handleDoneAction(_:)))
    
    let flexible = UIBarButtonItem(barButtonSystemItem: .flexibleSpace, target: nil, action: nil)
    actionBar.items = [clearButton,flexible, doneButton]
  ...
  @objc internal func handleClearAction(_: UIBarButtonItem) {
