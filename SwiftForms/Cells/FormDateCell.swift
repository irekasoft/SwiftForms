...
open class FormDateCell: FormValueCell {
  ...
  override func handleClearAction(_: UIBarButtonItem) {
    
    print("handleDoneAction")
  ...
  override func handleDoneAction(_: UIBarButtonItem) {
    ...
    rowDescriptor?.value = datePicker.date as AnyObject
    valueLabel.text = getDateFormatter().string(from: datePicker.date)
    update()
